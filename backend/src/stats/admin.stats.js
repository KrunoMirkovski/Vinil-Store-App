...
const Vinyl = require("../vinyls/vinyls.model");
...
router.get("/", async (req, res) => {
  try {
    ...
    // 4. Trending vinyls statistics:
    const trendingVinylsCount = await Vinyl.aggregate([
      { $match: { trending: true } }, // Match only trending books
      { $count: "trendingVinylsCount" }, // Return the count of trending books
    ]);

    // If you want just the count as a number, you can extract it like this:
    const trendingVinyls =
      trendingVinylsCount.length > 0
        ? trendingVinylsCount[0].trendingVinylsCount
        : 0;

    // 5. Total number of vinyls
